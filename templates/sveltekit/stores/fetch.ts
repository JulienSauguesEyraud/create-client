...
import { SubmissionError, type SubmissionErrors } from "../utils/types";
...
const normalizeAuth = (options: RequestInit): RequestInit => {
  if (
    auth &&
    options.headers instanceof Headers &&
...
export const fetchApi = (input: RequestInfo, init: RequestInit = {}) => {
  input = normalizeInput(input);
  init = [normalizeHeaders, normalizeContentType, normalizeAuth].reduce(
